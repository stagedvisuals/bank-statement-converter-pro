...
-- RLS Policies - Fix: gebruik email in plaats van role (aangenomen dat jij de admin bent)
...
-- Drop policies als ze bestaan
DROP POLICY IF EXISTS "Admin full access" ON agent_jobs;
DROP POLICY IF EXISTS "Admin full access retention" ON onboarding_retention_flows;
DROP POLICY IF EXISTS "Admin full access quality" ON scan_quality_flags;
DROP POLICY IF EXISTS "Admin full access market" ON market_trends;
DROP POLICY IF EXISTS "Admin full access reports" ON market_agent_reports;
DROP POLICY IF EXISTS "Users view own quality flags" ON scan_quality_flags;

-- Maak policies op basis van auth.uid() (alle ingelogde admins kunnen alles zien)
...
-- Function om logs toe te voegen
